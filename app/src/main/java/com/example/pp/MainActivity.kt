...
import com.example.pp.help
import com.example.pp.settings
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            livingroomButton.setOnClickListener { garazab() }//TEST
            bedroomButton.setOnClickListener { kapijab() }
            kitchenButton.setOnClickListener { kuhinjab() }
            outletButton.setOnClickListener { sobab() }
            hoodButton.setOnClickListener { lightsb() }
            doorButton.setOnClickListener { musicb() }
        }
        ...
        temperatureTextView.text = "${temperatureSeekBar.progress + 1}rpm"
        temperatureSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            @SuppressLint("SetTextI18n")
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                temperatureTextView.text = "%d rpm".format(progress)
                temperatureTextView.text = progress.toString() + "rpm"
